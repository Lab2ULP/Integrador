...
exports.addDiaNoLaborable = async (req, res) => {
  ...
  try {
    ...
    if (!nombre || !profesionalID || !diaNoLaborableID || !fecha) {
      await t.rollback(); // Deshacer la transacción si los datos están incompletos
      console.log("Faltan datos en la solicitud");
      return res
        .status(400)
        .json({ error: "Faltan datos para agregar el día no laborable" });
    }
...
exports.removeDiaNoLaborable = async (req, res) => {
  ...
  try {
    ...
    if (!ID) {
      await t.rollback(); // Deshacer la transacción si no hay ID
    ...
    if (deletedRows === 0) {
      await t.rollback(); // Deshacer la transacción si no se eliminó ninguna fila
