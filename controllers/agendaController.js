...
exports.actualizarTurnos = async (req, res) => {
  ...
  try {
    ...
    const turno = await Turno.findByPk(ID, { transaction: t });
    ...
    await console.error("Error al actualizar el turno:", error);
...
exports.crearAgenda = async (req, res) => {
  ...
  try {
    ...
    // Paso 2: Crear la clasificación extra si corresponde
    ...
    // Paso 3: Crear los días de agenda
    ...
    // Paso 4: Crear los turnos para cada día seleccionado
    for (const dia of dias) {
      ...
      while (fechaTurno <= new Date(sumarUnDia(fechaHasta))) {
        if (fechaTurno.getDay() === diaID) {
          // Turnos mañana
          if (dia.horaInicioManiana) {
            const turnosManiana = calcularTurnos(
              dia.horaInicioManiana,
              dia.horaFinalManiana,
              duracionTurnos
            );
          ...
          // Turnos tarde
          if (dia.horaInicioTarde) {
            const turnosTarde = calcularTurnos(
              dia.horaInicioTarde,
              dia.horaFinalTarde,
              duracionTurnos
            );
...
exports.crearSobreturno = async (req, res) => {
  ...
  try {
    ...
    await Agenda.update(
      ...
      { where: { ID: agendaID }, transaction: t }
    );
